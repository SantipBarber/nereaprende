import adjetivosTermina from "./assets/adjetivos_termina.svg";
import indefinidos from "./assets/indefinidos.svg";
import poetry from "./assets/poetry.svg";

export const content = [
    {
      title: "Los adjetivos con v y los prefijos de lugar",
      content: `
  # Las reglas de la V en los adjetivos
  ¿Sabías que los asturianos somos muy observadores? Del mismo modo que observamos la diferencia entre el verdor de los Picos de Europa y el azul del mar Cantábrico, vamos a aprender a diferenciar cuándo usar la v en los adjetivos.
  
  ## Regla de la V en adjetivos
  
  Se escriben con **v** los adjetivos que terminan en:
  - **-ave**: suave, grave
  - **-evo/-eva**: nuevo, nueva
  - **-avo/-ava**: octavo, esclava
  - **-ivo/-iva**: deportivo, activa
  
  ### Los prefijos de lugar
  
  Los prefijos de lugar nos ayudan a situar las cosas, como cuando explicamos dónde está el Naranco o la Plaza Mayor de Oviedo.
  
  Principales prefijos de lugar:
  - **sub-**: por debajo de (subsuelo)
  - **sobre-**: por encima de (sobrevolar)
  - **ante-**: delante de (antesala)
  - **extra-**: fuera de (extraescolar)
  <div class="my-2">
  > ¡Recuerda! Estos prefijos siempre indican posición o ubicación.`,
        image: adjetivosTermina
    },
    {
      title: "Numerales e indefinidos: ¡Contando historias!",
      content: `
  # Numerales e indefinidos
  
  ## Los numerales: Contando con precisión
  
  Los numerales son palabras que expresan cantidades u orden de manera exacta. Como cuando contamos los concejos de Asturias o el orden de llegada en la Subida al Naranco.
  
  ### Tipos de numerales:
  
  1. **Numerales cardinales**: 
     - Expresan cantidades precisas
     - Ejemplos: uno, dos, tres, cuatro, cinco...
     - *"En mi clase somos veinticinco alumnos"*
  
  2. **Numerales ordinales**:
     - Expresan orden
     - Ejemplos: primero, segundo, tercero...
     - *"Vivo en el tercer piso del edificio"*
  
  ## Los indefinidos: Cuando no necesitamos ser precisos
  
  Los indefinidos expresan cantidades de forma imprecisa, como cuando decimos que hay "muchos" peregrinos en el Camino de Santiago.
  
  ### Indefinidos más frecuentes:
  - algún/alguna
  - ningún/ninguna
  - varios/varias
  - pocos/pocas
  - muchos/muchas
  - bastantes
  - demasiados
  - cualquiera
  - nadie
  - alguien`,
        image: indefinidos
    },
    {
      title: "¡La poesía suena!",
      content: `
  # La poesía y sus rimas
  
  Los asturianos tenemos una rica tradición poética, desde las coplas tradicionales hasta los poemas modernos. ¡Vamos a descubrir cómo funciona la rima!
  
  ## ¿Qué es un poema?
  - Es un texto escrito en verso
  - Cada línea es un verso
  - Los versos suelen rimar entre sí
  
  ## Tipos de rima
  
  ### 1. Rima consonante
  - Coinciden todas las letras desde la vocal acentuada
  - Ejemplo: 
    *montaña* / *cabaña*
    *sidra* / *vidra*
  
  ### 2. Rima asonante
  - Solo coinciden las vocales desde la vocal acentuada
  - Ejemplo:
    *verde* / *fuente*
    *Nalón* / *calor*
  
  ### Estructura de un poema
  - Los poemas se organizan en versos
  - Varios versos forman una estrofa
  - Las estrofas forman el poema completo
  <div class="my-2">
  > ¡Dato curioso! Muchas canciones tradicionales asturianas son en realidad poemas con música, ¡y tienen rima!`,
        image: poetry
    }
  ];